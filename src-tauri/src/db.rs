...
/// dùng cho migration tự-chữa-lành vì ALTER TABLE ADD COLUMN không có
/// IF NOT EXISTS. `table` là identifier cố định từ code, không phải input user.
...
mod tests {
    ...
    /// (W59b DoD 1) DB "version 17 nhưng thiếu cloud_upload_state" (hậu quả
    /// đụng số version giữa các wave) → mở DB → bảng được tạo lại, version=18.
    #[test]
    fn migration_v18_recreates_missing_cloud_upload_state() {
        let dir = std::env::temp_dir().join(format!("browserx-db-test-{}", uuid::Uuid::new_v4()));
        let _guard = TempDir(dir.clone());
        std::fs::create_dir_all(&dir).unwrap();
        {
            // Dựng schema đầy đủ rồi mô phỏng DB hỏng: DROP bảng v15 + hạ
            // user_version về 17 (guard `< 15` sẽ bỏ qua vĩnh viễn).
            let db = Db::open_at_dir(&dir).unwrap();
            let conn = db.lock();
            conn.execute_batch("DROP TABLE cloud_upload_state").unwrap();
        ...
        // Bảng được tạo lại, dùng được ngay.
    ...
    /// (W59b DoD 2) DB lành mạnh v17 đầy đủ → migrate lên 18 không đổi gì
    /// ngoài version: profile, engine_version pin và upload state giữ nguyên.
    #[test]
    fn migration_v18_noop_on_healthy_v17_db() {
        let dir = std::env::temp_dir().join(format!("browserx-db-test-{}", uuid::Uuid::new_v4()));
        let _guard = TempDir(dir.clone());
        std::fs::create_dir_all(&dir).unwrap();
        let profile_id;
        {
            ...
            db.set_cloud_upload_failed(&p.id, "boom").unwrap();
            let conn = db.lock();
            conn.pragma_update(None, "user_version", 17).unwrap();
        }
        ...
        let version: i64 = db
            ...
            .unwrap();
        assert_eq!(version, 18);
        // engine_version pin KHÔNG bị backfill ghi đè; upload state còn nguyên.
        let p = db.get_profile(&profile_id).unwrap();
        assert_eq!(p.engine_version, "101.0.0.0");
        let s = db.get_cloud_upload_state(&profile_id).unwrap().unwrap();
        assert_eq!(s.status, "failed");
        assert_eq!(s.last_error.as_deref(), Some("boom"));
    }

    /// (W59b) Trường hợp nặng nhất: version=17 nhưng thiếu TOÀN BỘ DDL
    /// v15–v17 (schema chỉ tới v14) → v18 chữa cả bảng lẫn cột ALTER
    /// (transport/chat_id/engine_version) + backfill engine_version.
    #[test]
    fn migration_v18_heals_missing_v15_to_v17_ddl() {
        let dir = std::env::temp_dir().join(format!("browserx-db-test-{}", uuid::Uuid::new_v4()));
        let _guard = TempDir(dir.clone());
        std::fs::create_dir_all(&dir).unwrap();
        {
            ...
            for schema in [
                ...
                SCHEMA_V8, SCHEMA_V9, SCHEMA_V10, SCHEMA_V11, SCHEMA_V12, SCHEMA_V13, SCHEMA_V14,
            ] {
        ...
        assert_eq!(version, 18);
        // Bảng v15 dùng được.
        db.set_cloud_upload_started("old-1", "pending").unwrap();
        // Cột v16 (transport/chat_id) dùng được.
        ...
        .unwrap();
        assert_eq!(db.list_cloud_backups(Some("old-1")).unwrap().len(), 1);
