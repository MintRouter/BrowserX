...
- [ ] ≥2 profile khác seed → **canvas/WebGL hash khác nhau** (browserleaks).
- [ ] `navigator.webdriver=false`, UA **không** lộ `HeadlessChrome` (sannysoft).
- [ ] **PoC Rust** spawn được **1 CloakBrowser headful native** trên máy dev (macOS),
...
- [ ] UI/DB set được **≥12/17 flag** `--fingerprint*` (docs/07 §2) theo profile.
...
- [ ] Bulk tạo **≥500 profile** từ 1 template trong 1 thao tác; bulk launch/stop **≥20
...
- [ ] Automation: script mẫu qua CDP điều khiển ≥1 profile (goto + click + đọc DOM) thành công.
- [ ] **0 secret plaintext** trong DB & bản export (proxy/cookie) — xác nhận qua dump + scan.
- [ ] Lưu trữ **≥5.000 profile stored**; list/search local p95 < 300ms.
...
- [ ] **100%** hành động tạo/sửa/launch/stop sinh bản ghi **audit** truy vấn được.
- [ ] Lưu trữ **≥10.000 profile stored** không suy giảm list/search (p95 < 500ms).
- [ ] Dashboard/log hiển thị realtime: concurrent sessions, RAM/phiên, launch p95, error rate.
- [ ] Watchdog: tiến trình con crash → trạng thái cập nhật < 10s, **không** "profile treo
      running", không rác tiến trình.
- [ ] **Không** binary CloakBrowser trong installer/artifact (kiểm tra gói).
