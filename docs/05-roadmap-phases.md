...
- [ ] Hoàn tất **toàn bộ** checklist eval **docs/06 §8** (mọi mục ✅).
      — ⚠️ SUPERSEDED: eval Docker Manager đã pivot bỏ (dự án đi thẳng PoC native +
      Pha 1). Mục tiêu 'ghi baseline' được đáp ứng bằng docs/09-eval-baseline.md
      (native macOS, real-binary tests) thay cho checklist Docker docs/06 §8.
      Checklist §8 giữ lại làm tham chiếu, KHÔNG chạy.
...
- [x] `navigator.webdriver=false`, UA **không** lộ `HeadlessChrome` (sannysoft).
      — ✅ W31a 2026-07-07 test stealth.rs real-binary PASS: navigator.webdriver=false,
      UA Chrome/145 KHÔNG chứa 'Headless', navigator.plugins.length=5,
      navigator.languages.length=2, typeof window.chrome=object (commit e065740).
      ⚠️ mới assert tín hiệu cốt lõi qua CDP, CHƯA chạy full suite sannysoft.com.
...
- [x] Tài liệu baseline ghi rõ hạn chế môi trường (software GL, font; fingerprint-platform
      **mặc định** theo host nhưng chọn được tự do — cảnh báo mismatch khi cross-OS,
      docs/03 §6).
      — ✅ docs/09-eval-baseline.md tổng hợp version engine + stealth/fingerprint/
      data-integrity/proxy baseline + hạn chế môi trường (software GL, font,
      fingerprint-platform default theo host + cảnh báo cross-OS). Số liệu trích từ
      test THẬT (W34a).
